...
export const getId = createAsyncThunk('tickets/getId', async function (state) {
  ...
  reducers: {
    ...
    changeTickets(state, actions) {
      ...
      if (actions.payload === '2') {
        state.TICKET.sort(
          (a, b) => a.segments[0].stops.length - b.segments[0].stops.length
        )
        state.mostFree = false
        state.mostFust = false
        state.optimal = true
    ...
    filterTickets(state, actions) {
      ...
      function filterTicketsFunc(val) {
        if (actions.payload.checkdeStatus) {
          ...
          const newState = filterArr.filter(
            (el) =>
              el.segments[0].stops.length === val ||
              el.segments[1].stops.length === val
          )
          const oldState = JSON.parse(JSON.stringify(state.TICKET))
          newState.map((el) => oldState.unshift(el))
          state.TICKET = Array.from(
            new Set(oldState.map((obj) => JSON.stringify(obj)))
