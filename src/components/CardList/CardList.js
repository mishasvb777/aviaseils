...
const CardList = () => {
  ...
  return (
    <div className={styles.cardList}>
      ...
      {statusSearch === true && TICKET.length === 0 && <p className={styles.help_message}>Не найдено билетов по выбранным фильтрам, выберите фильтры</p>}
      {firstPageTickets && <h1>Выберите фильтры</h1>}
      {!firstPageTickets && TICKET === 0 && <h1>Выберите фильтры</h1>}      
      {!firstPageTickets && TICKET.slice(0, countCardsView).map(el =>         
          <Card ticket={el} key={key++}/>   
       )}      
      <button className={styles.showmore} onClick={toggleCountViewTickets}> Показать еще 5 билетов</button>
    </div>
  )
}
