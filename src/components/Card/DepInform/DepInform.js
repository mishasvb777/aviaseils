...
const DepInform = ({there, from}) => {
  ...
  const timeFrom = format(new Date(dateFromm.getTime() + there.duration * 60 * 1000), "HH:mm") 
    ...
    <>
      <div className={styles.card__body__inform}>
        <div className={styles.card_body__inform__section}>
          <span className={styles.card_body__inform__section__title}>{there.origin} - {from.origin}</span>
          <span className={styles.card_body__inform__section__value}>{dateThere} - {timeFrom}</span>
        </div>
        <div className={styles.card_body__inform__section}>
          <span className={styles.card_body__inform__section__title}>В пути</span>
          <span className={styles.card_body__inform__section__value}>{hours + 'ч ' + minuts + 'м'}</span>
        </div>      
        <div className={styles.card_body__inform__section}>
          <span className={styles.card_body__inform__section__title}>
            {there.length} {there.stops.length > 1 ? 'пересадки' : 'пересадка'} 
          </span>
          <span className={`${styles.card_body__inform__section__value} ${styles.uppercase}`}>{there.stops.join(' ')}</span>
        </div>      
      </div>      
    </>
